...
We can examine the relative performance of each method on a per-participant per training time basis:
...
bbmethods <- c("origPred", "OpenCVPred", "SetBBPred")

for(tt in unique(comparepredictions$traintime)){
  
  thistimepreds <- comparepredictions[comparepredictions$traintime == tt,]
  
  splitpreds <- split(thistimepreds, thistimepreds$participantCode)
  
  for(bbm in bbmethods){
    participantAccuracy <- lapply(splitpreds, 
                                  function(x){
                                    getAccuracy(table(x$observedAttentionName, 
                                                      x[,bbm]))}
    )
    
    accuracyresults <- rbind(accuracyresults,
                             data.frame(tt=tt,
                                        bbm=bbm,
                                        participantCode=names(participantAccuracy),
                                        accuracy=t(data.frame(participantAccuracy))
                             )
    )
  }
}

# get colours same as videos
accuracyresults$bbm <- factor(accuracyresults$bbm, 
                             levels=bbmethods[c(2,3,1)])


ggplot(data=accuracyresults, aes(x=tt, y=accuracy, colour = bbm)) +
  geom_line() + geom_point() + facet_wrap( ~ participantCode) + theme_bw()
