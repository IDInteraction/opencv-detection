...
# warning("Subsetting for testing")
...
for(tt in trainingtimes){
  ...
  modelFit <- randomForest(table1formula,
                           data = trainingdata)
  ...
  thisresult <- data.frame(method = "randomforest",
                           participant = p,
                           trainingtime = tt,
                           accuracy = accuracy)
...
RandForestBytimeresults <- sqldf("select method, trainingtime, avg(accuracy) as accuracy
                       from allresults
                       group by method, trainingtime")
RandForestAllResults <- allresults


save(table1formula, RandForestBytimeresults,
     RandForestAllResults, file="randomForests.RData")
